        ...
        <header className="mb-16 space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2 p-1.5 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-2xl rounded-[2rem] border border-white/20 dark:border-zinc-800 shadow-lg">
                {genres.map((g) => (
                  <button
                    key={g.id}
                    onClick={() => setActiveGenre(g.id)}
                    className={`px-6 py-2.5 rounded-full text-xs font-black uppercase tracking-tight transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-red-500
                      ${
                        activeGenre === g.id
                          ? 'bg-red-600 text-white shadow-lg shadow-red-600/40 scale-105'
                          : 'hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-500 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100'
                      }`}
                  >
                    {g.name}
                  </button>
